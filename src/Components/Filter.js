...
class Filter extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      min: 0,
      step: 5,
      currentValue: this.props.sliderValue
    };
  }
  ...
  createCheckbox = label => (
    <div key={this.hashCode(label)}>
      <Checkbox label={label} handleCheckboxChange={this.toggleCheckbox} />
    </div>
  );
   ...
   */
  createCheckboxes = () => this.props.producers.map(this.createCheckbox);
  ...
  render() {
    ...
    if (this.selectedCheckboxes.size !== 0) {
      emptyButton = (
        <Button id="emptybutton" onClick={this.emptyFilters}>
          Tyhjennä
        </Button>
      );
    }
